...
import { VenueHeroGallery, VenuePhotoSection } from "@/components/venue-photo-gallery";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return venue ? { title: `${venue.name} 대관·기술 정보 | MATRIX`, description: `${venue.name}의 대관료, 엔지니어, 오디오콘솔, PA, 백라인 정보를 확인하세요.` } : {};
}
  ...
  return <div className="matrix-detail-page">
    ...
    <main className="detail-main">
      <div className="detail-hero">
        <VenueHeroGallery images={venueImages} venueName={venue.name} accent={venue.accent} />
        <div className="detail-summary"><p className="detail-area">{venue.area}{venue.neighborhood ? ` · ${venue.neighborhood}` : ""}</p><h1>{venue.name}</h1><p className="detail-type">{venue.type}</p><p className="detail-address"><MapPin />{venue.address}</p><div className="detail-summary-grid">{venue.capacity && <div><span>수용 인원</span><strong>{venue.capacity.toLocaleString("ko-KR")}명</strong></div>}{representative !== null && <div><span>대표 대관료</span><strong>{formatWon(representative)}부터</strong></div>}{venue.staff.soundEngineer !== null && <div><span>음향 엔지니어</span><strong>{venue.staff.soundEngineer ? "포함" : "미포함"}</strong></div>}{venue.audio.console && <div><span>오디오콘솔</span><strong>{venue.audio.console}</strong></div>}</div></div>
      </div>
      <div className="detail-layout"><div className="detail-content">
        ...
        <VenuePhotoSection images={venueImages} venueName={venue.name} />
        <Section title="인력" rows={[{ label: "음향 엔지니어", value: included(venue.staff.soundEngineer) }, { label: "모니터 엔지니어", value: included(venue.staff.monitorEngineer) }, { label: "조명 엔지니어", value: included(venue.staff.lightingOperator) }, { label: "무대 스태프", value: included(venue.staff.stageStaff) }, { label: "기본 포함", value: venue.staff.includedNotes }, { label: "추가 비용", value: venue.staff.extraFee }, { label: "비고", value: venue.staff.notes }]} />
        <Section title="음향" rows={[{ label: "오디오콘솔", value: venue.audio.console }, { label: "메인 PA", value: venue.audio.mainPa }, { label: "모니터", value: venue.audio.monitor }, { label: "유선 마이크", value: venue.audio.wiredMic }, { label: "무선 마이크", value: venue.audio.wirelessMic }, { label: "DI 박스", value: venue.audio.diBox }]} />
        <Section title="백라인" rows={[{ label: "드럼", value: venue.backline.drums }, { label: "기타앰프", value: venue.backline.guitarAmp }, { label: "베이스앰프", value: venue.backline.bassAmp }, { label: "건반 / 피아노", value: venue.backline.keyboard }, { label: "비고", value: venue.backline.notes }]} />
        <Section title="조명" rows={[{ label: "조명기", value: venue.lighting?.fixtures }, { label: "무빙 라이트", value: venue.lighting?.movingLights }, { label: "팔로우 스폿", value: venue.lighting?.followSpot }, { label: "조명 콘솔", value: venue.lighting?.console }, { label: "비고", value: venue.lighting?.notes }]} />
