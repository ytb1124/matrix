...
export function VenueExplorer({ initialVenues, naverMapClientId, initialFilters = {} }: { initialVenues: Venue[]; naverMapClientId: string; initialFilters?: Record<string, string> }) {
  ...
  const [capacityMin, setCapacityMin] = useState(Number(initialFilters.capacityMin) > 0 ? Number(initialFilters.capacityMin) : 0);
  ...
  useEffect(() => {
    ...
    if (maxPrice < PRICE_MAX) params.set("maxPrice", String(maxPrice)); if (capacityMin > 0) params.set("capacityMin", String(capacityMin));
  ...
  }, [query, area, dayBasis, maxPrice, capacityMin, engineer, drums, guitarAmp, bassAmp, parking, waitingRoom, consoleOnly, sort, view]);
  ...
  }, [initialVenues, query, area, dayBasis, maxPrice, capacityMin, engineer, drums, guitarAmp, bassAmp, parking, waitingRoom, consoleOnly, sort]);
  ...
  const activeCount = Number(Boolean(query)) + Number(area !== "전체") + Number(priceActive) + Number(capacityMin > 0) + Number(engineer) + Number(drums) + Number(guitarAmp) + Number(bassAmp) + Number(parking) + Number(waitingRoom) + Number(consoleOnly);
  ...
  const filters = <><label className="filter-select">지역<select value={area} onChange={(event) => setArea(event.target.value)}>{areas.map((item) => <option key={item}>{item}</option>)}</select><ChevronDown /></label><label className="filter-select">가격 기준<select value={dayBasis} onChange={(event) => setDayBasis(event.target.value as DayBasis)}>{dayOptions}</select><ChevronDown /></label><label className="filter-select">최소 인원<select value={capacityMin} onChange={(event) => setCapacityMin(Number(event.target.value))}><option value="0">전체</option><option value="50">50명</option><option value="100">100명</option><option value="150">150명</option><option value="300">300명</option></select><ChevronDown /></label><div className="price-filter-popover"><PriceControl value={maxPrice} onChange={setMaxPrice} /></div><button className={`filter-button ${engineer ? "selected" : ""}`} onClick={() => setEngineer(!engineer)}>Engineer</button><button className={`filter-button ${drums ? "selected" : ""}`} onClick={() => setDrums(!drums)}>Drum</button><button className={`filter-button ${guitarAmp ? "selected" : ""}`} onClick={() => setGuitarAmp(!guitarAmp)}>Guitar</button><button className={`filter-button ${bassAmp ? "selected" : ""}`} onClick={() => setBassAmp(!bassAmp)}>Bass</button><button className={`filter-button ${consoleOnly ? "selected" : ""}`} onClick={() => setConsoleOnly(!consoleOnly)}>Console</button><button className={`filter-button ${parking ? "selected" : ""}`} onClick={() => setParking(!parking)}>주차</button>{activeCount > 0 && <button className="filter-reset" onClick={reset}>전체 초기화</button>}</>;
  ...
  return <div className="matrix-page">
    ...
    {mobileFilters && <div className="mobile-sheet-backdrop" onMouseDown={(event) => event.target === event.currentTarget && setMobileFilters(false)}><div className="mobile-sheet"><header><h2>필터</h2><button onClick={() => setMobileFilters(false)}><X /></button></header><label>지역<select value={area} onChange={(event) => setArea(event.target.value)}>{areas.map((item) => <option key={item}>{item}</option>)}</select></label><label>가격 기준<select value={dayBasis} onChange={(event) => setDayBasis(event.target.value as DayBasis)}>{dayOptions}</select></label><label>최소 수용 인원<select value={capacityMin} onChange={(event) => setCapacityMin(Number(event.target.value))}><option value="0">전체</option><option value="50">50명</option><option value="100">100명</option><option value="150">150명</option><option value="300">300명</option></select></label><PriceControl value={maxPrice} onChange={setMaxPrice} /><div className="mobile-options"><button className={engineer ? "selected" : ""} onClick={() => setEngineer(!engineer)}>FOH Engineer</button><button className={drums ? "selected" : ""} onClick={() => setDrums(!drums)}>Drum</button><button className={guitarAmp ? "selected" : ""} onClick={() => setGuitarAmp(!guitarAmp)}>Guitar Amp</button><button className={bassAmp ? "selected" : ""} onClick={() => setBassAmp(!bassAmp)}>Bass Amp</button><button className={consoleOnly ? "selected" : ""} onClick={() => setConsoleOnly(!consoleOnly)}>FOH Console</button><button className={parking ? "selected" : ""} onClick={() => setParking(!parking)}>주차</button><button className={waitingRoom ? "selected" : ""} onClick={() => setWaitingRoom(!waitingRoom)}>대기실</button></div><div className="mobile-sheet-actions"><button onClick={reset}>초기화</button><button onClick={() => setMobileFilters(false)}>{filtered.length}곳 보기</button></div></div></div>}
    <WelcomeDialog />
  </div>;
}
